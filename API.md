...
Отправляет запрос, при котором веб-сервер принимает данные, заключённые в тело сообщения, для хранения в firebase
...
## Внимание:
Если `title` не задан, то все параметры (`course`, `semestr`, `teacher`, `subject`) должны быть не `null`

Если `title` задан, то эти параметры могут существовать, а могут и не существовать
